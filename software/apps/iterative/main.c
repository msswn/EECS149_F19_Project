...
int main(void)
{
  ...
  float spotLength = l + 2.4 * d1;
  float spotWidth = l * 1.25;
  float spotBack = -spotLength / 2;
  float spotFront = spotLength / 2;
  float spotLeft = spotWidth / 2;
  float spotRight = -spotWidth / 2;
  ...
  f1 = {S - k / sqrt(1 + pow(k, 2)) * Rmin, H - (1 - 1 / sqrt(1 + pow(k, 2))) * Rmin};
  f1 = f1 + {xf, yf};
  f2 = {k / sqrt(1 + pow(k, 2)) * Rmin, (1 - 1 / sqrt(1 + pow(k, 2))) * Rmin};
  f2 = f2 + {xf, yf};
  ...
  if (f2[1] < spotBack + d1)
  {
    ...
    float dS[2] = f1 - f2;
    float dSn[2] = {dS[2] - dS[1]} / norm(dS);
    float cx = Rmin * dSn[1] + f2[1];
    float cy = Rmin * dSn[2] + f2[2];
  ...
  else
  {
    ...
    f4 = {0, Rmin - sqrt(pow(Rmin, 2) - pow(d1, 2))};
    f4 = f4 + newO;
  ...
  float f6xd = f5[1] + sqrt(pow(Rmin, 2) - pow(Rmin - f5[2], 2));
  float f6y = 0;
  f6 = {f6xd, f6y};
  ...
  if (f6[1] >= spotFront - d1)
  {
    ...
    f8 = {f7(1) - sqrt(pow(Rmin,2) - pow(Rmin - f7(2),2)), 0};
    f9 = spotCenter;
  }
  else
  {
  ...
  print("f1: %f, f2: %f, f4: %f, f6: %f, f7: %f, f8: %f, f9: %f",f1,f2,f4,f5,f6,f8,f9);
